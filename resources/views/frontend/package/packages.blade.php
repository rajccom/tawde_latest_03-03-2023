                    ...
                    <div class="overflow-hidden shadow-none border-right">
                        <div class="card-body">
                            ...
                            <div class="text-center">
                                @if ($package->id != 1)
                                    @if(Auth::check())
                                        <a href="{{ route('package_payment_methods', encrypt($package->id)) }}" type="submit" class="btn btn-primary" >{{translate('Enroll This Package')}}</a>
                                    @else
                                        <button type="submit" onclick="loginModal()" class="btn btn-primary" >{{translate('Enroll This Package')}}</button>
                                    @endif
                                @else
                                    <a href="javascript:void(0);" class="btn btn-light" ><del>{{translate('Enroll This Package')}}</del></a>
                                @endif
                            </div>
                        </div>
                    </div>
