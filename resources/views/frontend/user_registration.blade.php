...
<div class="col-lg-10 col-xl-8 col-xxl-10 mx-auto text-center">
 <h1>{{ translate('Register Now') }}</h1>
 	<p>Let's start your partner search with this Sign up.</p>
 </div></div>
</div>
							...
							<div class="row">
						        <div class="col-lg-12">
						            <div class="form-group mb-3">
										...
										<input type="text" class="form-control @error('first_name') is-invalid @enderror" name="first_name" id="first_name" placeholder="{{translate('Name')}}"  oninput="this.value = this.value.replace(/[^a-zA-Z ]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" minlength="3" maxlength="32" required>
										@error('first_name')
											<span class="invalid-feedback" role="alert">{{ $message }}</span>
										@enderror
						            </div>
			                       ...
			                       <div>
									<div class="d-flex justify-content-between align-items-start">
										...
										<button class="btn btn-link p-0 opacity-50 text-reset fs-12" type="button" onclick="toggleEmailPhone(this)">{{ translate('Use Phone Instead') }}</button>
									</div>
									<div class="form-group phone-form-group mb-1 d-none">
							            <input type="tel" id="phone-code" class="form-control{{ $errors->has('phone') ? ' is-invalid' : '' }}" value="{{ old('phone') }}" placeholder="Enter your number " name="phone" autocomplete="off" onKeyDown="if ((event.keyCode >= 48 &amp;&amp; event.keyCode <= 57) || (event.keyCode == 8) || (event.keyCode == :sunglasses: || (event.keyCode == 46) || (event.keyCode >= 189 &amp;&amp; event.keyCode <= 190) || (event.keyCode >= 35 &amp;&amp; event.keyCode <= 40) || (event.keyCode >= 96 &amp;&amp; event.keyCode <= 105) || (event.keyCode >= 109 &amp;&amp; event.keyCode <= 110) || event.keyCode == 9  || event.keyCode == 13) { return true; } else { return false; }" maxlength="10" pattern="[6789][0-9][9]*" minlength="10" required>
							        </div>

							        <input type="hidden" name="country_code" value="">

							        <div class="form-group email-form-group mb-1 ">
							            <input type="email" class="form-control {{ $errors->has('email') ? ' is-invalid' : '' }}" value="{{ old('email') }}" placeholder="{{  translate('Email') }}" name="email"  autocomplete="off" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" required>
							            @if ($errors->has('email'))
							...
							@else
									<div class="col-lg-12">
									  <div class="form-group mb-3">
											...
											<input type="email" class="form-control @error('email') is-invalid @enderror" name="email" id="signinSrEmail" placeholder="{{ translate('Email Address') }}" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" required >
									        @error('email')
									            <span class="invalid-feedback" role="alert">{{ $message }}</span>
									        @enderror
									  </div>
							...
							<div class="row">
								<div class="col-lg-6">
									<div class="form-group mb-3">
										...
										<input type="password" class="form-control @error('password') is-invalid @enderror" name="password" placeholder="********" aria-label="********" minlength="8"  required>
								...
								<div class="col-lg-6">
									<div class="form-group mb-3">
										...
										<input type="password" class="form-control" name="password_confirmation" placeholder="********" minlength="8"  required>
										<small>{{ translate('Minimun 8 characters') }}</small>
									</div>
								</div>
							</div>
