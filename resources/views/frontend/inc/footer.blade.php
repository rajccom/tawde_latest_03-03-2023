<footer class="aiz-footer fs-14 mt-5 text-white fw-400 pt-3">
    <div class="container">

        <div class="row mb-4">
            <div class="col-xxl-6 col-xl-7 col-lg-8 col-md-10 text-center mx-auto">
                <div class="logo mb-2 footer-logo">
                    <a href="{{ route('home') }}" class="d-inline-block py-0px">
                        @if(get_setting('footer_logo') != null)
                            <img src="{{ uploaded_asset(get_setting('footer_logo')) }}" alt="{{ env('APP_NAME') }}" class="mw-100 h-100px h-md-100px" height="40" width="110">
                        @else
        ...
        <div class="mb-4">
            <h4 class="text-uppercase fs-14 border-bottom border-primary pb-2 mb-3">{{ translate('Contacts') }}</h4>
        ...
        <div class="row no-gutters">
            ...
            <div class="col-xl col-md-6 mb-4">
                <h4 class="text-uppercase fs-14 border-bottom border-primary pb-2 mb-3">{{ get_setting('widget_one_title') }}</h4>
                <div>
                    <ul class="list-unstyled">
                        @foreach (json_decode( get_setting('widget_one_labels'), true) as $key => $value)
                            <li class="my-1">
                                <a href="{{ json_decode( get_setting('widget_one_links'), true)[$key] }}" class="text-reset">{{ $value }}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
            ...
            <div class="col-xl col-md-6 mb-4">
                <h4 class="text-uppercase fs-14 border-bottom border-primary pb-2 mb-3">{{ get_setting('widget_two_title') }}</h4>
                <div>
                    <ul class="list-unstyled">
                        @foreach (json_decode( get_setting('widget_two_labels'), true) as $key => $value)
                            <li class="my-1">
                                <a href="{{ json_decode( get_setting('widget_two_links'), true)[$key] }}" class="text-reset">{{ $value }}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
            ...
            <div class="col-xl col-md-6 mb-4">
                <h4 class="text-uppercase fs-14 border-bottom border-primary pb-2 mb-3">{{ get_setting('widget_three_title') }}</h4>
                <div>
                    <ul class="list-unstyled">
                        @foreach (json_decode( get_setting('widget_three_labels'), true) as $key => $value)
                            <li class="my-3">
                                <a href="{{ json_decode( get_setting('widget_three_links'), true)[$key] }}" class="text-reset">{{ $value }}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
            ...
            <div class="col-xl col-md-6 mb-4">
                <h4 class="text-uppercase fs-14 border-bottom border-primary pb-2 mb-3">{{ get_setting('widget_mobile_app_title') }}</h4>
                <div class="mb-3">
        ...
        <div class="border-top border-primary pt-4 pb-7 pb-xl-4">
            <div class="row">
                <div class="col-lg-8">
                    <!--<div class="mb-3">
                        <i class=""></i>
                        <span>{{ translate('Tawdevivaha Matrimonial Website') }}</span>
                    </div>-->
                    <div class="lh-1">
                        {!! get_setting('footer_copyright_text') !!} | Made with ❤ by <a href="https://ccomsys.net/" target="_blank">{{ translate(' C COM DIGITAL') }}</a>
                    </div>
                </div>
...
<div class="aiz-mobile-bottom-nav d-xl-none fixed-bottom bg-white shadow-lg border-top rounded-top" style="box-shadow: 0px -1px 10px rgb(0 0 0 / 15%)!important; ">
    <div class="row align-items-center gutters-5 text-center">
        <div class="col">
            <a href="{{ route('dashboard') }}" class="text-reset d-block flex-grow-1 text-center py-2">
                <i class="las la-home fs-18 {{ areActiveRoutes(['dashboard'],'opacity-100')}}"></i>
                <span class="d-block fs-10 {{ areActiveRoutes(['dashboard'],'opacity-100 fw-600')}}">{{ translate('Dashboard') }}</span>
            </a>
        </div>
        <div class="col">
            <a href="{{ route('frontend.notifications') }}" class="text-reset d-block flex-grow-1 text-center py-2">
                <span class="d-inline-block position-relative px-2">
                    <i class="las la-bell fs-18 {{ areActiveRoutes(['frontend.notifications'],'opacity-100')}}"></i>
                ...
                <span class="d-block fs-10 {{ areActiveRoutes(['frontend.notifications'],'opacity-100 fw-600')}}">{{ translate('Notifications') }}</span>
            </a>
        </div>
        <div class="col">
          <a href="{{ route('all.messages') }}" class="text-reset d-block flex-grow-1 text-center py-2 {{ areActiveRoutes(['all.messages'],'opacity-100')}}">
              <span class="d-inline-block position-relative px-2">
                  <i class="las la-comment-dots fs-18 {{ areActiveRoutes(['all.messages'],'opacity-100')}}"></i>
                    @if(Auth::check() && Auth::user()->user_type == 'member')
                        @php
              ...
              <span class="d-block fs-10 {{ areActiveRoutes(['all.messages'],'opacity-100 fw-600')}}">{{ translate('Messages') }}</span>
          </a>
        </div>
        @if (Auth::check())
            @if(Auth::user()->user_type == 'member')
                <div class="col">
                    <a href="javascript:void(0)" class="text-reset d-block flex-grow-1 text-center py-2 mobile-side-nav-thumb" data-toggle="class-toggle" data-target=".aiz-mobile-side-nav">
                        ...
                        <span class="d-block fs-10">{{ translate('Account') }}</span>
                    </a>
                </div>
            @else
                <div class="col">
        ...
        @else
            <div class="col">
                <a href="{{ route('login') }}" class="text-reset d-block flex-grow-1 text-center py-2">
                    <span class="d-block mx-auto mb-1 {{ areActiveRoutes(['login'],'opacity-100')}}">
                        <img src="{{ static_asset('assets/img/avatar-place.png') }}" class="rounded-circle size-20px">
                    </span>
                    <span class="d-block fs-10 {{ areActiveRoutes(['login'],'opacity-100 fw-600')}}">{{ translate('Account') }}</span>
                </a>
            </div>
        @endif
    </div>
