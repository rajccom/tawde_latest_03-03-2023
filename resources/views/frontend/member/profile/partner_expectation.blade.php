        ...
        <form action="{{ route('partner_expectations.update', $member->id) }}" method="POST">
            ...
            <div class="form-group row">
                <div class="col-md-6">
                    ...
                    <input type="number" name="partner_height" value="{{ !empty($member->partner_expectations->height) ? $member->partner_expectations->height : "" }}" step="any"  placeholder="{{ translate('Min Height') }}" class="form-control" required>
                    @error('partner_height')
                        <small class="form-text text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="col-md-6">
                    <label for="partner_height_max">{{translate('Max Height')}}  ({{ translate('In Feet') }})</label>
                    <input type="number" name="partner_height_max" value="{{ !empty($member->partner_expectations->height_max) ? $member->partner_expectations->height_max : "" }}" step="any"  placeholder="{{ translate('Max Height') }}" class="form-control" required>
            ...
            <div class="col-md-6">
                    ...
                    <select class="form-control aiz-selectpicker" name="partner_diet" required>
                        ...
                        <option value="vegetarian" @if($partner_dieting_acceptable ==  'vegetarian') selected @endif >{{translate('Vegetarian')}}</option>
                        <option value="eggetarian" @if($partner_dieting_acceptable ==  'eggetarian') selected @endif >{{translate('Eggetarian')}}</option>
                        <option value="occasionally-non-veg" @if($partner_dieting_acceptable ==  'occasionally-non-veg') selected @endif >{{translate('Occasionally Non Veg')}}</option>
                        <option value="non-vegetarian" @if($partner_dieting_acceptable ==  'non-vegetarian') selected @endif >{{translate('Non-Vegetarian')}}</option>
                    </select>
                    @error('partner_diet')
                        <small class="form-text text-danger">{{ $message }}</small>
                    @enderror
