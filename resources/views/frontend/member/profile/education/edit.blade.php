<form action="{{ route('education.update', $education->id) }}" method="POST">
    ...
    <div class="modal-body">
        ...
        <div class="form-group row">
            ...
            <div class="col-md-9">
                <input type="date" name="education_start" value="{{$education->start}}" class="form-control" placeholder="{{translate('Start Date')}}" required>
            </div>
        </div>
        <div class="form-group row">
            <label class="col-md-3 col-form-label">{{translate('End Date')}}</label>
            <div class="col-md-9">
                <input type="date" name="education_end" value="{{$education->ends}}"  placeholder="{{ translate('End Date') }}" class="form-control" >
            </div>
        </div>
    </div>
