          ...
          <div class="card-body">
            <form action="{{ route('happy-story.update', $happy_story->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <textarea name="details" class="aiz-text-editor form-control" data-buttons='[["font", ["bold", "underline", "italic"]],["para", ["ul", "ol", "paragraph"]],["view", ["undo","redo"]]]' placeholder="Type.." data-min-height="200">{{ $happy_story->details }}</textarea>
                    @error('details')
                        <small class="form-text text-danger">{{ $message }}</small>
                    @enderror
                </div>
